...
Automation Rules v1.0.4 builds responsive graphical AND/OR conditions over cached `ospy.provider.v1` values and enabled built-in OSPy sensors from Pihrt or Shelly. Ultrasonic sensors provide distance, water level, fill and volume conditions. It supports confirmation and repeat intervals, recovery notices, a separate default-on test state, bounded action history, Home popups, explicitly permitted browser notifications on every authenticated OSPy page, e-mail, Telegram and mobile push. Boolean settings use accessible sliding switches placed directly beside their labels, all editor controls provide localized hover help, and an explicit confirmed action sends a real test through the currently selected channels without evaluating conditions. It executes no irrigation control actions.
