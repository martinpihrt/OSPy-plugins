...
Automation Rules v1.0.5 builds responsive graphical AND/OR conditions over cached `ospy.provider.v1` values, enabled built-in OSPy sensors from Pihrt or Shelly, and local date/time values with same-day or overnight range comparisons. Ultrasonic sensors provide distance, water level, fill and volume conditions. Trigger, repeat and recovery messages include actual values, operators and limits. Browser permission now performs an immediate visible test and falls back to a Service Worker where needed. It also supports confirmation and repeat intervals, recovery notices, a separate default-on test state, bounded action history, Home popups, e-mail, Telegram and mobile push. It executes no irrigation control actions.
