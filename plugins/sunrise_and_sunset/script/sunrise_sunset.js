...
function sun_display_update_schedule() {
    if ($('#displayScheduleDate').length > 0) {
        $.get( "/plugins/sunrise_and_sunset/fetch_data", {"date" : toXSDate(displayScheduleDate)}, function( sun_data ) {
            $(".stationSchedule .scheduleTick").each( function() {
                var cellTime = parseInt($(this).attr("data")) * 60;
                if (cellTime < sun_data.sunrise + 60) {
                    var x = 0;
                    var w = Math.max(0,Math.min(100,(sun_data.sunrise - cellTime)/60*100));
                    $(this).css({
                        "background-image" : background_svg(x, w),
                        "background-size" : "cover"
                    });
                } else if (cellTime > sun_data.sunset - 60) {
                    var x = Math.max(0,Math.min(100,(sun_data.sunset - cellTime)/60*100));
                    var w = 100;
                    $(this).css({
                        "background-image" : background_svg(x, w),
                        "background-size" : "cover"});
...
jQuery(document).ready(function(){
    ...
    if (window.location.pathname == "/") {
        if ($('#displayScheduleDate').length > 0) {
            ...
            observer = new MutationObserver(sun_display_update_schedule);
