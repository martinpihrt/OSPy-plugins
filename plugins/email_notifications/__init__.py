            ...
            try:
                ...
                if email_options["emlrain"]:
                    if inputs.rain_sensed() and not last_rain:
                        body += '<b>' + _('System') + '</b> ' + datetime_string() 
                        body += '<br><p style="color:red;">' + _('System detected rain.') + '</p><br>'
...
class settings_page(ProtectedPage):
    ...
    def GET(self):
        try:
            ...
            msg += _('email_notifications -> settings_page GET')
    ...
    def POST(self):
        try:
            ...
            log.error(NAME, _('E-mail plug-in') + ':\n' + traceback.format_exc())
            msg = _('An internal error was found in the system, see the error log for more information. The error is in part:') + ' '
            msg += _('email_notifications -> settings_page POST')
            return self.core_render.notice('/', msg)
...
class help_page(ProtectedPage):
    ...
    def GET(self):
        try:
            ...
            log.error(NAME, _('E-mail plug-in') + ':\n' + traceback.format_exc())
            msg = _('An internal error was found in the system, see the error log for more information. The error is in part:') + ' '
            msg += _('email_notifications -> help_page GET')
            return self.core_render.notice('/', msg)
