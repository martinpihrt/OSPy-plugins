...
void twi_stop(void)
{
  ...
  //
  // XXX we can't use SB_WAIT here as SB_WAIT attempts to reset TWI, calling
  // twi_stop in the process, resulting in a recursive call.
  twi_loopcnt = 0;
  while (TWCR & _BV(TWSTO) && twi_loopcnt < twi_maxloops) {
    twi_loopcnt += 1;
  }
