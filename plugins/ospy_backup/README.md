...
lifecycle and `health()` interfaces. Its former one-shot worker was removed
because backups are created synchronously on request. Concurrent backup
requests are rejected, and a stop request is checked between copied plug-in
data directories. Diagnostics reports an active backup, the latest successful
archive name and size, and the latest failure or cancellation.
