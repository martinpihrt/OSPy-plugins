...
Before changing the Git working tree, the plug-in also starts an external update watchdog. On systemd installations it runs in a separate transient service, so it remains active while OSPy restarts. The new OSPy process confirms the update only after a fresh scheduler heartbeat and the web interface starts listening. If a healthy start is not confirmed within 120 seconds, the watchdog resets the repository to the previous commit and restarts OSPy automatically. Systems without systemd use a detached watchdog process. If the watchdog cannot be started, the update is aborted before tracked files are changed.
